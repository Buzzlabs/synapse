    ...
    async def check_password(self, user_id, password):
        ...
        if not (await self.account_handler.check_user_exists(user_id)):
            ...
            if localpart != localpart.lower() and self.regLower:
                logger.info('User %s cannot be created due to lowercase policy', localpart)
                return False
        ...
        user_obj = UserID.from_string(user_id)

        if auth.get("profile"):
            logger.info("Handling profile data")
            profile = auth["profile"]

            store = self.account_handler._hs.get_datastores().main
            ...
            if "display_name" in profile and (
                ...
                logger.info("Setting display name to '%s'", display_name)
                await store.set_profile_displayname(user_obj, display_name)

            if self.config.updateThreepid and "three_pids" in profile:
